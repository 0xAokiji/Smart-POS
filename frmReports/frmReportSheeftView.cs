...
{
    public partial class frmReportSheeftView : SampleView
    {
        public frmReportSheeftView()
        {
            InitializeComponent();
        }
                ...
                using (SqlConnection con1 = MainClass.GetConnection())
                {
                    ...
                    using (SqlCommand cmd1 = new SqlCommand(qry1, con1))
                    {
                        ...
                        if (dt1.Rows.Count > 0)
                        {
                            ...
                            string qry = @"SELECT amountSafe.ID, staff.sName, amountSafe.Amount, amountSafe.aTime, amountSafe.aDate 
                                   FROM amountSafe 
                                   JOIN staff ON amountSafe.nameID = staff.staffID
                                   WHERE staff.sName LIKE N'%" + txtSearch1.Text + "%' AND amountSafe.aDate BETWEEN @StartDate AND @EndDate";
        ...
        private void guna2DataGridView2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (guna2DataGridView2.CurrentCell.OwningColumn.Name == "dgvdel")
            {
                ...
                if (guna2MessageDialog1.Show(" هل تريد حذه هذا الصنف ") == DialogResult.Yes)
                {
                    ...
                    string qry = "Delete from amountSafe where ID = " + id + "";
